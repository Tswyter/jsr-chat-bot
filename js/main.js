...
const processResponse = (input) => {
  ...
  if (convoLevel === 0 && checkFor('yes') || checkFor('yep') || checkFor('absolutely') || checkFor('of course') || checkFor('you know it') || checkFor('sure')) {
    ...
    xhr.open('GET', `https://swapi.co/api/people/?search=${sanitizedInput}`);
    xhr.send();
    xhr.onload = () => {
      if (xhr.status < 300) {
        ...
        if (foundChars.length > 1) {
          ...
          presentOptions([...names, 'Load more characters?']);
    ...
    xhr.onerror = () => {
      console.log('Oops there seems to be an error with the request.', xhr.responseText);
